...
/* Arcade port! Expects to run through keyboard and SNES pad. Adds Key */
/* config on startup, coin input, and reduced resolution  */
/* 16 Sep 2000 */
...
#define textout_center textout_centre
...
int SEEALLEND;
int KeyMoveUp, KeyMoveDown, KeyMoveLeft, KeyMoveRight;
int KeyFireUp, KeyFireDown, KeyFireLeft, KeyFireRight;
int KeyCredits, KeyStart, Credits, StartPressed;
int intFrame, base;
...
col1.r=sinewave[cycle_red];
col1.g=sinewave[cycle_green];
col1.b=sinewave[cycle_blue];

col2.r=sinewave[cycle_red+SINESTEP];
col2.g=sinewave[cycle_green+SINESTEP];
col2.b=sinewave[cycle_blue+SINESTEP];

col3.r=sinewave[cycle_red+SINESTEP2];
col3.g=sinewave[cycle_green+SINESTEP2];
col3.b=sinewave[cycle_blue+SINESTEP2];

intFrame=~intFrame;

/* processing done, get a vsync and set stuff up */
...
/* a stretch blit is fast and it works */

if (intFrame) {
  stretch_blit(tmpscreen, screen, 1, 1, 640,480, 0, 0, 320, 240);
} else {
  stretch_blit(tmpscreen, screen, 0, 0, 640,480, 0, 0, 320, 240);
}

/* check for start */
readpad(0);
if (snes.start) simulate_keypress(561);   // '1'
...
{ RGB col;
 ...
 set_color_depth(8);
 if (set_gfx_mode(GFX_AUTODETECT,320,240,320,240))
   fail("Can not set VGA 320x240x256 graphics mode\n");
 ...
 /* Internally, the game will still run at full resolution */
 /* Eh,<shrugs> we have the horsepower to spare. */
 ...
 tmpscreen=create_bitmap(640,480); /* game screen, copied in interrupt */
 if (tmpscreen==NULL) fail("Need memory for tmpscreen\n");

 set_palette(data[palette].dat);

 create_rgb_table(&myMap, data[palette].dat, NULL);
 rgb_map=&myMap;
 ...
 if (!NOCYCLE)
 { if (install_int(cycle,16)) /* was 20 */
...
gameloop:
 clear_to_color(tmpscreen,black);
 StartPressed=0;
...
t_start:
  ...
  if ((StartPressed) && (Credits>0)) goto exit;
  ...
  blit(work2,tmpscreen,0,0,0,0,640,480);
  ...
  { retrace_count=0;
    ...
    blit(work,tmpscreen,0,SCREENOFFY,0,0,640,480);
  ...
  { if (scroll_text("~1Takatron: 2098")) goto exit;
    ...
    if (scroll_text("Copyrighted by ~3M.Brent~1, 98-00")) goto exit;
    if (scroll_text("http://tursi.yiffco.com")) goto exit;
    if (scroll_text("tursi@yiffco.com")) goto exit;
    if (scroll_text(" ")) goto exit;
    if (scroll_text("Written using ~2DJGPP~1 and ~3Allegro 3.0")) goto exit;
    if (scroll_text(" ")) goto exit;
    if (Credits>0) {
      if (scroll_text("~2Press Start~1 to begin game...")) goto exit;
    } else {
      if (scroll_text("~2Insert Coin~1 to begin game...")) goto exit;
    }
  } else {
...
{ int i; char buf[1024];
  ...
  for (i=0; i<40; i++)
  { retrace_count=0;
    blit(work,tmpscreen,35,100,35,200,570,250);
    blit(work,work,35,101,35,100,570,350);
    rect(work,35,449,605,451,black);
    call_joystick();
    if ((StartPressed) && (Credits>0)) i=100;
    if (key[KEY_ESC]) i=100;
    rand();
    sprintf(buf, "Credits: %d", Credits);
    textout_centre(tmpscreen,data[Small_Font].dat,buf,320,465,white);
...
{ retrace_count=0;
  ...
  draw_sprite(tmpscreen,work2,0,0);
...
{ retrace_count=0;
  k=(i*3)/4;
  rectfill(tmpscreen,(320-i),(240-k),(320+i),(240+k),black);
  while (retrace_count<1);
}
...
{ int k;
  ...
  if (snes.down) DOWN=1;
  if (snes.left) LEFT=1;
  if (snes.right) RIGHT=1;
  readpad(1);
  if (snes.up) FIREUP=1;
  if (snes.down) FIREDOWN=1;
  if (snes.left) FIRELEFT=1;
  if (snes.right) FIRERIGHT=1;
...
set_clip(tmpscreen,30,0,609,479);
...
{ retrace_count=0;
  ...
  set_clip(tmpscreen,30,0,609,449);
  blit(work,tmpscreen,0,SCREENOFFY,0,0,640,480);
  set_clip(tmpscreen,30,30,609,449);
...
{ fancy_clear();
  ...
  set_clip(tmpscreen,30,30,609,449);
  blit(tmpscreen,work2,30,30,30,30,610,450);
  px=320;
  py=240;
  an=1;
  for (index=0; index<MAXBULL; index++)
  ...
  textout_centre(tmpscreen,data[MyFont].dat,buf,320,150,254);
...
{ /* Simba bites it */
  ...
  { sprintf(buf2,"Continue? (%d)",Continues);
    textout_centre(tmpscreen,data[MyFont].dat,buf2,320,150,254);
    ...
    { retrace_count=0;
      buf[0]=time+'0';
      textout_centre(tmpscreen,data[MyFont].dat,buf,320,180,253);
      while (retrace_count<50)
      { 
        if ((StartPressed)&&(Credits>0)) time=-1;
      }
      time--;
    }
    if (time<0)
    { retrace_count=0;
      textout_centre(tmpscreen,data[MyFont].dat,"              ",320,150,black);
      textout_centre(tmpscreen,data[MyFont].dat," ",320,180,black);
...
{ /* that's all, folks */
  ...
  textout_centre(tmpscreen,data[MyFont].dat,"GAME OVER",320,150,254);
  call_joystick();
  while (FIRE)
    call_joystick();
  retrace_count=0;
...
fancy_clear();
textout_centre(tmpscreen,data[Times].dat,"Use Move to Select",320,50,253);
textout_centre(tmpscreen,data[Times].dat,"Use Fire Up to Enter",320,100,253);
fl=0;
i=0;
strcpy(str, " A ");

while (FIREUP) call_joystick();

k='A';
while ((i<9)&&(fl==0))
{
  FIREUP=0;
  while (!FIREUP) {  /* get one letter */
    str[1]=k;
    textout_center(tmpscreen,data[Times].dat, str, 320, 260, white);
    rest(50);
    call_joystick();
    if (UP) {
      k--;
  ...
  rectfill(tmpscreen,40,200,600,300,black);
  buf[i]=0;
  textout_centre(tmpscreen,data[MyFont].dat,buf,320,200,white);
}
i=9;
while ((Score>hi_score[i-1])&&(i>0))
  i--;
sprintf(buf2,"Number %d score!",i+1);
textout_centre(tmpscreen,data[Times].dat,buf2,320,400,254);
if (i<9)
  for (j=9; j>=i; j--)
...
{ char buf[10];
  ...
  /* sorry, no config */
  /* pad 1=move, pad 2=shoot, 5=coin, 1=start, esc=quit (on attract mode) */
...
{ int *intp,i;
  ...
  { while (!feof(fp))
    { fgets(string,80,fp);
      ...
      if (toupper(string[i])=='C') intp=&KeyCredits;
      if (toupper(string[i])=='S') intp=&KeyStart;
...
{ int index;
  ...
  textout_centre(tmpscreen,data[MyFont].dat,"CONGRATULATIONS",320,125,254);
...
int keycall(int inkey)
{
  if (inkey>>8 == KeyCredits>>8) {
    Credits++;
    StartPressed=0;
    play_sample(data[FOLLOW].dat,255,128,1000,0);
  }

  if (inkey>>8 == KeyStart>>8) {
    StartPressed=1;
  }
